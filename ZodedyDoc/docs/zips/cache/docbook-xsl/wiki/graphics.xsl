...
<!-- ********************************************************************
     $Id: graphics.xsl,v 1.2 2008/08/01 01:04:37 dcarver Exp $
     ********************************************************************
...
<xsl:param name="wiki.use.image.tag" select="1"/>
<xsl:param name="wiki.image.begin"><xsl:text>[[Image:</xsl:text></xsl:param>
<xsl:param name="wiki.image.end"><xsl:text>]]</xsl:text></xsl:param>
  ...
  <xsl:variable name="img">
    <xsl:choose>
      <xsl:when test="@format = 'SVG'">
         <!-- Do any wiki's handle SVG??? -->
      </xsl:when>
      <xsl:otherwise>
        <xsl:choose>
            <xsl:when test="$wiki.use.image.tag = 1">
                <xsl:value-of select="$wiki.image.begin"/>
                 <xsl:choose>
                   <xsl:when test="$img.src.path != '' and
                                 $tag = 'img' and
                  ...
                  <xsl:value-of select="$wiki.image.end"/>
            </xsl:when>
            <xsl:otherwise>
                <xsl:text>&lt;img src="{$output_filename}"/></xsl:text>
            </xsl:otherwise>
        </xsl:choose>
      </xsl:otherwise>
    </xsl:choose>
...
<!-- Override and fill in for specific wikis -->
</xsl:template>
  ...
  <xsl:choose>
    <xsl:when test="@format='linespecific'">
      <xsl:choose>
        ...
        <xsl:otherwise>
          <xsl:text>&lt;a xlink:type="simple" xlink:show="embed" xlink:actuate="onLoad"
             href="{$filename}"/></xsl:text>
        </xsl:otherwise>
      </xsl:choose>
    </xsl:when>
    <xsl:otherwise>
...
<xsl:template name="longdesc.link">
  ...
  <xsl:variable name="href.to">
    <xsl:call-template name="trim.common.uri.paths">
      ...
      <xsl:text>[</xsl:text>
      <xsl:text>&lt;a href="{$href.to}" target="longdesc"></xsl:text>D<xsl:text>&lt;/a></xsl:text>
      <xsl:text>]</xsl:text>
</xsl:template>
